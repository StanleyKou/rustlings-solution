...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        let mut temp_left = scores.get(&team_1_name);
        if temp_left.is_none() {
            ...
            scores.insert(team_1_name, temp2);
        } else {
            let temp2 = temp_left.unwrap();
        ...
        let mut temp_right = scores.get(&team_2_name);
        if temp_right.is_none() {
            let temp2 = Team {
                name: team_2_name.clone(),
                goals_scored: team_2_score.clone(),
                goals_conceded: team_1_score.clone(),
            };
            scores.insert(team_2_name, temp2);
        } else {
            let temp2 = temp_right.unwrap();
